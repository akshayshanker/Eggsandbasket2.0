...
class LifeCycleParams:
		...
		def __init__(self,\
			...
			self.param_id =  ''.join(random.choices(string.ascii_uppercase + string.digits, k=6))+'_'+time.strftime("%Y%m%d-%H%M%S") +'_'+mod_name
			self.mod_name = mod_name

			if random_draw == False:
				self.og_DB = LifeCycleModel(param_dict, np.array([0]), self.param_id,mod_name = mod_name)
				self.og_DC = LifeCycleModel(param_dict, np.array([1]), self.param_id,mod_name = mod_name)
			...
			if random_draw == True:
				...
				self.og_DB = LifeCycleModel(param_dict_new, np.array([0]), self.param_id,mod_name = mod_name)
				self.og_DC = LifeCycleModel(param_dict_new, np.array([1]), self.param_id,mod_name = mod_name)
