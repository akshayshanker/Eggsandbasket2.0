 ...
 #                                         self.parameters.beta_m)
    ...
    class ModelCartGrids:
        ...
        def __init__(self, grid1d, stochgrids, param):
            ...
            dc_low_errors_sd_1 = (np.log(self.Q_DC_shocks[:,0]) - r_l)/l*sigma_d
            dc_low_errors_sd_1 = dc_low_errors_sd_1\
                                    /np.sqrt((np.inner(dc_low_errors_sd_1**2, self.Q_DC_P)))

            # Construct the log-normally distribution r_m_prime shock vector 
            self.r_m_prime = np.exp(mu_lnmort + sd_lnmort * dc_low_errors_sd_1)
